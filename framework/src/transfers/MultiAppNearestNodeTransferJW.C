...
#include "MultiAppNearestNodeTransferJW.h"
...
registerMooseObject("MooseApp", MultiAppNearestNodeTransferJW);

InputParameters
MultiAppNearestNodeTransferJW::validParams()
{
  ...
  params.addParam<Real>("nearest_multiplier",
                        1.1,
                        "Value to multiply nearest distance used for "
                        "including quad points in the averaging before sending "
                        );
...
MultiAppNearestNodeTransferJW::MultiAppNearestNodeTransferJW(const InputParameters & parameters)
  : MultiAppConservativeTransfer(parameters),
    ...
    _nearest_multiplier(getParam<Real>("nearest_multiplier")),
...
void
MultiAppNearestNodeTransferJW::execute()
{
  TIME_SECTION(
      "MultiAppNearestNodeTransferJW::execute()", 5, "Transferring variables based on nearest nodes");
            ...
            if (current_distance < (_cached_min_dist[pid][qp] * _nearest_multiplier)) // TODO:, if distance < min_distance*2
            {
                  ...
                  0)
              {
                ...
                if (_fixed_meshes)
                  ...
                  printf("adding to cache pid: %i  qp: %i i_local_from: %i i_node: %i, from_dof:%i, val: %f, _cached_min_dist: %f, current dist: %f, size cache: %i \n",
                    pid, qp, i_local_from, i_node, from_dof, outgoing_evals[2*qp+1], _cached_min_dist[pid][qp], current_distance, _cached_nearby_from_dof_dists[pid][qp].size()
                  );
      ...
      for (auto & elem : to_mesh.active_local_element_ptr_range())
      {
        ...
        for (auto & problem_from : _cached_froms)
        {
          ...
          for (MooseIndex(points) offset = 0; offset < points.size(); offset++)
          {
            ...
            // unsigned int qp_ind = node_index_map[pid][key];
            printf("getting nearest_pts (size is %u), pid %i, qp %i \n", _cached_nearby_from_dof_ids[pid].size(), pid, qp );
...
Real
MultiAppNearestNodeTransferJW::bboxMaxDistance(const Point & p, const BoundingBox & bbox)
{
...
Real
MultiAppNearestNodeTransferJW::bboxMinDistance(const Point & p, const BoundingBox & bbox)
{
  std::array<Point, 2> source_points = {{bbox.first, bbox.second}};
...
void
MultiAppNearestNodeTransferJW::getLocalEntitiesAndComponents(
    MooseMesh * mesh,
...
MultiAppNearestNodeTransferJW::getTargetLocalNodes(const unsigned int to_problem_id)
{
